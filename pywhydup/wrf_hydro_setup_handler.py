...
class WrfHydroSetup(object):
    ...
    def overwrite_rainfall_forcing_data(self, rainrate_dataarray):
        ...
        fn_list = glob(path.join(self.absolute_path,
                                 self.forcing_dir,
                                 '*LDASIN*'))
        fn_list.sort()
...
class SetupDirHandler(object):
    def __init__(self, root_dir, template_dir, template_forcing_dir='forcing'):
        ...
        self.template_forcing_dir = template_forcing_dir
    ...
    def duplicate_template(self,
        ...
        try:
            ...
            fn_list_base_dir = glob(path.join(self.root_dir,
                                              self.template_dir,
                                              '*'))
            ...
            shutil.copytree(path.join(self.root_dir, self.template_dir, 'lib'),
                            path.join(self.root_dir, new_dir, 'lib'))
            print('Copying template forcing dir content...')
            fn_forcing_list = self._get_forcing_file_list(t_start=t_start,
                                                          t_stop=t_stop,
                                                          t_format=t_format)

            if not path.exists(path.join(self.root_dir,
                                         new_dir,
                                         self.template_forcing_dir)):
    ...
    def _get_forcing_file_list(self,
                               t_start=None,
                               t_stop=None,
                               t_format='%Y-%m-%d'):
        if t_start:
